...
- Window-scoped stats come from a new server-side function `get_account_chatter_stats_window(p_account_id, p_user_id, p_start, p_end)` returning `total`, `days`, `avg_per_day`, `mass_dms`, `oldest_chat`. Same admin/self auth check as the existing `get_account_chatter_stats`; all aggregation stays in the database, no row fetching in the browser.
