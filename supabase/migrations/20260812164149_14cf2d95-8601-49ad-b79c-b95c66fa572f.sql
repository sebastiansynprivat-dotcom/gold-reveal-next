...
BEGIN
  ...
  -- Serialize assignment generation briefly so simultaneous dashboard opens
  -- cannot all receive the same currently least-used targets.
  ...
  -- Keep old rows visible after the switch to account-grouped lists.
      ...
      SELECT v_user, ranked.id, v_today, v_acc.id
        FROM (
          SELECT t.id,
            ...
            LEFT JOIN (
              SELECT target_id, count(*) AS assignment_count
            ...
            LEFT JOIN (
              SELECT target_id, count(*) AS assignment_count
                FROM public.brezzels_comment_assignments
               WHERE assigned_date BETWEEN v_today - 6 AND v_today
               GROUP BY target_id
            ) week_usage ON week_usage.target_id = t.id
            LEFT JOIN (
              SELECT target_id,
                     count(*) AS times_seen,
                     max(assigned_date) AS last_seen
                FROM public.brezzels_comment_assignments
               WHERE user_id = v_user
               GROUP BY target_id
            ) user_usage ON user_usage.target_id = t.id
           WHERE t.active = true
             -- Only prevent duplicates for this chatter today. Other chatters
             -- are deliberately allowed to receive the same target.
             AND NOT EXISTS (
               SELECT 1
