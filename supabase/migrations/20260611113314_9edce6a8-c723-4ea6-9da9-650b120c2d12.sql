DROP FUNCTION IF EXISTS public.get_chatter_real_stats(uuid[]);

CREATE OR REPLACE FUNCTION public.get_chatter_real_stats(
  p_user_ids uuid[],
...
BEGIN
  ...
  WITH targets AS (
    ...
    UNION
    SELECT (SELECT p.user_id FROM profiles p WHERE p.id = pp LIMIT 1) AS uid, pp AS pid
    FROM unnest(p_profile_ids) AS pp
  ),
  asg AS (
    SELECT t.uid, t.pid, aa.account_id, aa.start_date, COALESCE(aa.end_date, v_today) AS end_date, aa.end_date IS NULL AS is_open
    FROM targets t
    JOIN account_assignments aa
      ON (t.uid IS NOT NULL AND aa.user_id = t.uid) OR (t.pid IS NOT NULL AND aa.profile_id = t.pid)
  ),
  rev AS (
    SELECT a.uid, a.pid, ad.date, SUM(ad.total)::numeric AS total
    FROM asg a
    JOIN accounts_data ad
      ON ad.account_id = a.account_id
     AND ad.date >= a.start_date
     AND ad.date <= a.end_date
    GROUP BY a.uid, a.pid, ad.date
  ),
  rev_agg AS (
    SELECT
      r.uid, r.pid,
    ...
    FROM rev r
    GROUP BY r.uid, r.pid
  ),
  open_latest AS (
    SELECT DISTINCT ON (a.uid, a.pid, a.account_id) a.uid, a.pid, a.account_id, ad.mass_dms, ad.unread_chats, ad.oldest_chat
    FROM asg a
    JOIN accounts_data ad ON ad.account_id = a.account_id
    WHERE a.is_open
    ORDER BY a.uid, a.pid, a.account_id, ad.date DESC
  ),
  activity AS (
    SELECT
      ol.uid, ol.pid,
    ...
    FROM open_latest ol
    GROUP BY ol.uid, ol.pid
  ),
  weeks AS (
    SELECT generate_series(0, 7) AS wi
  ),
  weekly AS (
    SELECT t.uid, t.pid, w.wi,
      (v_spark_start + (w.wi * 7))::date AS week_start,
      COALESCE(SUM(r.total), 0) AS total
    FROM targets t
    CROSS JOIN weeks w
    LEFT JOIN rev r ON (r.uid IS NOT DISTINCT FROM t.uid) AND (r.pid IS NOT DISTINCT FROM t.pid)
      AND r.date >= (v_spark_start + (w.wi * 7))::date
      AND r.date <  (v_spark_start + ((w.wi + 1) * 7))::date
    GROUP BY t.uid, t.pid, w.wi
  ),
  spark AS (
    SELECT wk.uid, wk.pid, jsonb_agg(jsonb_build_object('week_start', wk.week_start, 'total', wk.total) ORDER BY wk.wi) AS sparkline
    FROM weekly wk
    GROUP BY wk.uid, wk.pid
  )
  SELECT
    t.uid,
    t.pid,
  ...
  FROM targets t
  LEFT JOIN rev_agg ra ON (ra.uid IS NOT DISTINCT FROM t.uid) AND (ra.pid IS NOT DISTINCT FROM t.pid)
  LEFT JOIN activity ac ON (ac.uid IS NOT DISTINCT FROM t.uid) AND (ac.pid IS NOT DISTINCT FROM t.pid)
  LEFT JOIN spark sp ON (sp.uid IS NOT DISTINCT FROM t.uid) AND (sp.pid IS NOT DISTINCT FROM t.pid);
END;
$function$;