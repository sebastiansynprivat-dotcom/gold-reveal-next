...
BEGIN
  ...
  -- Only serialize duplicate/concurrent requests for this user. The previous
  -- global daily lock unnecessarily queued every chatter behind every other one.
  ...
  -- Preserve old same-day rows created before account-specific lists existed.
  ...
  LOOP
    ...
    IF v_needed > 0 THEN
      ...
      ON CONFLICT ON CONSTRAINT brezzels_comment_assignments_user_id_target_id_assigned_dat_key
      DO NOTHING;
    END IF;
  END LOOP;
