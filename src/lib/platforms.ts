...
const DEFAULT_SEED: PlatformDef[] = [
  { label: "Maloum",   key: "maloum",   color: "#d4af37", isActive: true, autoSynced: true, sortOrder: 10, styles: styleForIndex(0) },
...
export async function loadPlatformsFromDB(): Promise<void> {
  try {
    const { data, error } = await (supabase.from("platforms") as any)
      .select("id, key, label, color, is_active, sort_order, auto_synced")
    ...
    const mapped: PlatformDef[] = data
      ...
      .map((row: any, i: number) => ({
        ...
        autoSynced: row.auto_synced !== false,
