...
import ModelManager from "./CreateModel";
...
type PlatformFilter = "all" | "4based" | "maloum" | "brezzels" | "Fansyme";
type SubFilter =
  | "none"
  | "botdm_fehlt"
  | "botdm_vorhanden"
...
const PLATFORM_LABELS: PlatformFilter[] = ["all", "4based", "maloum", "brezzels", "Fansyme"];
...
function BotDmTable({
  ...
  rows: {
    acc: Account;
    dash: ModelDashboardRow | undefined;
    botMsg: BotMessageRow | undefined;
  }[];
  ...
  const getField = (acc: Account, type: "botdm" | "welcome" | "massdm") => {
    ...
    if (type === "botdm")
      return p === "Maloum" ? "maloum_botdm_done" : p === "Brezzels" ? "brezzels_botdm_done" : "fourbased_botdm_done";
    if (type === "welcome")
      return p === "Maloum" ? "maloum_submitted" : p === "Brezzels" ? "brezzels_submitted" : "fourbased_submitted";
...
export default function ModelDashboardTab() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) setAllBotMessages(data as BotMessageRow[]);
      });
  }, [loadAllDashboards]);

  const loadModelData = useCallback(async (accountId: string) => {
    setLoading(true);
    const { data } = await supabase.from("model_dashboard").select("*").eq("account_id", accountId).maybeSingle();
  ...
  const generateModelLogin = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://${projectId}.supabase.co/functions/v1/create-model-login`, {
        ...
        body: JSON.stringify({ account_id: selectedAccountId, email: selectedAccount?.account_email }),
      });
    ...
    <div className="space-y-5">
      ...
      >
        <div className="px-4 py-3 header-gradient-border flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-semibold text-foreground tracking-wide">Alle Accounts</h2>
        ...
        <div className="px-3 overflow-y-auto max-h-[500px]">
          {/* Sticky Header */}
          {/* <div className="grid grid-cols-[1fr_80px_80px_50px] gap-0 bg-accent/10 border-b border-accent/20"> */}
          <div className="sticky top-0 z-10 grid grid-cols-[1fr_80px_80px_50px] gap-0 bg-accent/10 border-b border-accent/20">
            ...
            <div className="flex items-center gap-3 px-1">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {selectedAccount?.account_domain && `${selectedAccount.account_domain} · `}
                ...
                <div className="space-y-2">
                  {(
                    [
                      {
                        label: "4Based",
                        value: fourbasedRevenue,
                        onChange: setFourbasedRevenue,
                      },
                      {
                        label: "Maloum",
                        value: maloumRevenue,
                        onChange: setMaloumRevenue,
                      },
                      {
                        label: "Brezzels",
                        value: brezzelsRevenue,
                        onChange: setBrezzelsRevenue,
                      },
                    ] as const
                  ).map((p) => (
                    <div
                  ...
                  <div className="space-y-2">
                    <div className="rounded-xl border border-accent/20 bg-accent/5 p-3 text-center space-y-1">
                      ...
                      <p className="text-2xl font-bold text-accent tabular-nums">
                        <AnimatedGoldValue
                          value={Math.round((effectiveMonthly * revenuePercentage) / 100)}
                          suffix={` ${currency}`}
                        />
                      </p>
                    </div>
                    ...
                    {totalPlatformRevenue > 0 && (
                      <div className="rounded-lg bg-secondary/20 border border-border/40 p-2.5 space-y-1.5">
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">
                          Aufschlüsselung Payout
                        </p>
                        {fourbasedRevenue > 0 && (
                          <div className="flex justify-between text-xs">
                            <span className="text-muted-foreground">4Based</span>
                            <span className="font-mono text-foreground">
                              {Math.round((fourbasedRevenue * revenuePercentage) / 100).toLocaleString("de-DE")}{" "}
                              {currency}
                            </span>
                          </div>
                        )}
                        {maloumRevenue > 0 && (
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="font-mono text-foreground">
                              {Math.round((maloumRevenue * revenuePercentage) / 100).toLocaleString("de-DE")} {currency}
                            </span>
                          </div>
                        )}
                        {brezzelsRevenue > 0 && (
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="font-mono text-foreground">
                              {Math.round((brezzelsRevenue * revenuePercentage) / 100).toLocaleString("de-DE")}{" "}
                              {currency}
                            </span>
                          </div>
                        )}
