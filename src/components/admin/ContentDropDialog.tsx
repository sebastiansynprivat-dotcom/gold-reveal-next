...
import { Search, Sparkles, Link2, Users, Send, Loader2, Check } from "lucide-react";
...
export default function ContentDropDialog({ open, onOpenChange }: Props) {
  ...
  const submit = async () => {
    ...
    if (!link.trim()) return toast.error("Content-Link fehlt");
    setSubmitting(true);
    try {
      ...
      toast.success(
        `Content gesendet an ${data?.recipients ?? 0} Chatter${(data?.recipients ?? 0) === 1 ? "" : ""}`
      );
      onOpenChange(false);
    } catch (e: any) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl glass-card border-accent/20 p-0 gap-0 overflow-hidden">
        <div className="px-6 pt-6 pb-4 border-b border-border/50 bg-gradient-to-b from-accent/5 to-transparent">
          <DialogHeader className="space-y-1">
            ...
            <DialogDescription className="text-xs">
              Wähle das Model, füge den Content-Link ein – alle aktiven Chatter auf allen Plattformen
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground flex items-center gap-2">
              <Link2 className="h-3 w-3" /> 2. Content-Link
            </label>
            <Input
        ...
        <div className="px-6 py-4 border-t border-border/50 bg-secondary/20 flex items-center justify-between gap-3">
          ...
          <button
            onClick={submit}
            disabled={submitting || !selected || !link.trim() || totalChatters === 0}
