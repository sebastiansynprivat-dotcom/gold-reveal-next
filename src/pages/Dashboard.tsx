...
function AnimatedDecimalValue({
  ...
  const display = (animated / 100).toLocaleString("de-DE", {
...
export default function Dashboard() {
  ...
  // useEffect(() => {
  //   if (!user) return;
  //   supabase.from("user_roles").select("role").eq("user_id", user.id).eq("role", "user").maybeSingle()
  //     .then(({ data }) => {
  //       if (data) {
  //         null
  //       } else {
  ...
  //dev2
  // async function getDetails(telegramId) {
  //   const response = await fetch(`api/maloum/reports/${telegramId}`, {
  //     headers: {
  ...
  //     setGroupName("Regular");
  //     setGroupNameSaved(true);
  //     setTelegramLoading(false);

  //     setUmsatz(0);

  //     setYesterdayRevenue(Math.round(profile.sum.revenue));

  //     setMonthlyRevenue(Math.round(profile.monthly[0]?.sum ?? 0));

  //     setTotalRevenue(Number(profile.allTimeRevenue.toFixed(2)));
  //   }
  // }, [profile]);
  ...
  useEffect(() => {
    ...
    loadRevenue();
  }, [user, telegramSaved, groupNameSaved]);
  ...
  const saveRevenue = useCallback(
    async (amount: number) => {
      ...
      const { error } = await supabase
        .from("daily_revenue")
        .upsert({ user_id: user.id, date: today, amount }, { onConflict: "user_id,date" });
      if (error) {
  ...
  if (!display) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-background">
        ...
        <p className="text-muted-foreground text-sm text-center mb-6">Du hast keine Chatter-Berechtigung.</p>
        <Button
          variant="outline"
          onClick={() => {
            localStorage.clear();
            ...
            <div className="hidden sm:flex items-center gap-3">
              ...
              <div className="ml-auto flex items-center gap-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="input-gold-shimmer rounded-lg">
                    <Input
                      ...
                      step={50}
                      value={umsatz || ""}
                      onChange={(e) => handleUmsatzChange(Number(e.target.value) || 0)}
                      placeholder="Umsatz €"
                      className="h-7 text-xs w-24 font-semibold border-transparent"
                    />
                  </div>
                </div>
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto flex-col p-1" align="end">
                    <div>
            ...
            <div className="flex sm:hidden flex-col gap-3">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <Popover>
                  ...
                  <PopoverContent className="text-xs w-auto flex-col p-1" align="end">
                    <ul>
                      <li>name</li>
                      <li>email</li>
                      <li className="hover underline cursor-pointer flex gap-1 items-center">
                        <LogOut onClick={signOut} className="h-3.5 w-3.5 mt-2" />
                      </li>
                    </ul>
                  </PopoverContent>
                </Popover>
                ...
                <div className="shrink-0 flex items-center gap-1" data-tour="revenue-input">
                  ...
                  <div className="input-gold-shimmer rounded-lg">
                    <Input
                      ...
                      onChange={(e) => handleUmsatzChange(Number(e.target.value) || 0)}
                      placeholder="€"
                      className="h-7 text-xs w-20 font-semibold border-transparent"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="border-t border-border/30">
              {demoModelInactive || assignedAccounts.some((acc) => acc.model_active === false) ? (
                ...
                <ModelRequestDialog
                  ...
                  modelLanguage={
                    assignedAccounts.length > 0 ? (assignedAccounts[0] as any).model_language || "de" : "de"
                  }
                />
              )}
            </div>
